...
async function seed() {
    ...
    try {
        ...
        if (managers.length === 0) {
            ...
            const [addressResult] = await conn.query("INSERT INTO EmployeeAddress (employee_id, street, city, state, zip) VALUES (?, ?, ?, ?, ?)", [employeeResult.insertId, superEmployee.street, superEmployee.city, superEmployee.state, superEmployee.zip]);
            const [phoneResult] = await conn.query("INSERT INTO EmployeePhone (employee_id, phone_num) VALUES (?, ?)", [employeeResult.insertId, superEmployee.phone_num]);
